...
# --- Set Up Workspace ---
save_folder = 'frames_social'
os.makedirs(save_folder, exist_ok=True)

# --- Step 1: Simulate Base Social Graph (Barabási–Albert Model) ---
G_base = nx.barabasi_albert_graph(300, 3)  # 300 nodes, each new node attaches to 3 existing nodes

print(f"Social Graph: {G_base.number_of_nodes()} nodes, {G_base.number_of_edges()} edges")
...
with imageio.get_writer('social_graph_melting.gif', mode='I', duration=0.3) as writer:
    for filename in frame_files:
        image = imageio.imread(filename)
        writer.append_data(image)

print("\n✅ Social graph melting animation saved as 'social_graph_melting.gif'!")
