...
# --- Set Up Workspace ---
save_folder = 'frames_knowledge'
os.makedirs(save_folder, exist_ok=True)

# --- Step 1: Build Simulated Knowledge Graph ---
G_base = nx.Graph()

# Define clusters (concept groups)
clusters = {
    'animals': [f'animal_{i}' for i in range(20)],
    'vehicles': [f'vehicle_{i}' for i in range(20)],
    'tools': [f'tool_{i}' for i in range(20)],
    'emotions': [f'emotion_{i}' for i in range(20)]
}

# Add strong internal links within clusters
for group in clusters.values():
    for i in range(len(group)):
        for j in range(i+1, len(group)):
            if np.random.rand() < 0.4:  # 40% chance internal connection
                G_base.add_edge(group[i], group[j])

# Add weak cross-cluster links
cluster_names = list(clusters.keys())
for _ in range(50):
    cluster_a, cluster_b = random.sample(cluster_names, 2)
    node_a = random.choice(clusters[cluster_a])
    node_b = random.choice(clusters[cluster_b])
    if not G_base.has_edge(node_a, node_b):
        G_base.add_edge(node_a, node_b)

print(f"Knowledge Graph: {G_base.number_of_nodes()} nodes, {G_base.number_of_edges()} edges")
...
with imageio.get_writer('knowledge_graph_melting.gif', mode='I', duration=0.3) as writer:
    for filename in frame_files:
        image = imageio.imread(filename)
        writer.append_data(image)

print("\n✅ Knowledge graph melting animation saved as 'knowledge_graph_melting.gif'!")
