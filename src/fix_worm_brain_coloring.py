...
# --- Set Up Workspace ---
save_folder = 'frames_worm_fixed'
os.makedirs(save_folder, exist_ok=True)

# --- Step 1: Regenerate Worm Brain Graph (Same as before) ---
...
# --- Step 2: Extract Base Snowflakes ---
...
# --- Step 4: Build New Animation (Optional) ---

frame_files = sorted([os.path.join(save_folder, f) for f in os.listdir(save_folder) if f.endswith('.png')])

with imageio.get_writer('worm_graph_melting_fixed.gif', mode='I', duration=0.3) as writer:
    for filename in frame_files:
        image = imageio.imread(filename)
        writer.append_data(image)

print("\n✅ Worm brain frames regenerated with drift coloring!") 